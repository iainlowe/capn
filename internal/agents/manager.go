...
// AgentFactory interface for creating agents
type AgentFactory interface {
	CreateAgent(id, name string, agentType AgentType) (Agent, error)
}

// DefaultAgentFactory creates basic agents
type DefaultAgentFactory struct{}

// NewDefaultAgentFactory creates a new default agent factory
func NewDefaultAgentFactory() *DefaultAgentFactory {
	return &DefaultAgentFactory{}
}

// CreateAgent creates an agent of the specified type
func (f *DefaultAgentFactory) CreateAgent(id, name string, agentType AgentType) (Agent, error) {
	switch agentType {
	case AgentTypeFile, AgentTypeNetwork, AgentTypeResearch:
		// For crew agents, we'll use the base agent for now
		// In a full implementation, this would delegate to crew package
		return NewBaseAgent(id, name, agentType), nil
	case AgentTypeCaptain:
		return NewBaseAgent(id, name, agentType), nil
	default:
...
// SetFactory sets the agent factory
func (m *AgentManager) SetFactory(factory AgentFactory) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.factory = factory
}
...
func (m *AgentManager) SpawnAgent(id, name string, agentType AgentType) (Agent, error) {
	...
	// Create the appropriate agent type using factory
	agent, err := m.factory.CreateAgent(id, name, agentType)
	if err != nil {
